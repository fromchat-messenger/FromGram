...
К сожалению, код микросервиса `file-server` закрытый. Я не знаю что внутри, там может быть бэкдор или все что угодно. В целях безопасности я отключил этому микросервису доступ к интернету. Это решение автора MyTelegram, я или автор Opengram не виноваты.